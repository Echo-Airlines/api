...
exports.executeVirtualAirlinesFlightsSync = executeVirtualAirlinesFlightsSync;
const prisma_1 = require("../../../prisma/generated/prisma/index.js");
const async_1 = require("async");
async function executeVirtualAirlinesFlightsSync(scheduler) {
...
async function executeVirtualAirlineFlightsSync(virtualAirline, scheduler) {
    return new Promise(async (resolve, reject) => {
        const oaFlights = await scheduler.services.OnAir.getVirtualAirlineFlights(virtualAirline.Id);
        if (oaFlights.length === 0) {
            scheduler.logger.warn(`No flights found for virtual airline ${virtualAirline.Id}`);
            return resolve(null);
        }
        const results = [];
        await (0, async_1.eachSeries)(oaFlights, async (flight) => {
            const entity = await executeVirtualAirlineFlightSync(flight, virtualAirline, scheduler);
            results.push(entity);
            return true;
        }, (err) => {
            if (err) {
...
async function executeVirtualAirlineFlightSync(dto, virtualAirline, scheduler) {
    let flightStatus = prisma_1.FlightStatus.PENDING;
    if (dto.Registered === false && dto.StartTime) {
        if (dto.Aircraft.AircraftStatus === 4) {
            flightStatus = prisma_1.FlightStatus.WARP;
        }
        else {
            flightStatus = prisma_1.FlightStatus.FLIGHT;
        }
    }
    else if (dto.Registered === true && dto.CancelReason) {
        flightStatus = prisma_1.FlightStatus.CANCELLED;
    }
    else if (dto.Registered === true && !dto.CancelReason) {
        flightStatus = prisma_1.FlightStatus.COMPLETED;
    }
    const query = {
        Id: dto.Id,
    ...
    const entity = await scheduler.services.Flight.upsert(query);
    if (!entity) {
...
//# sourceMappingURL=executeVirtualAirlinesFlightsSync.js.map